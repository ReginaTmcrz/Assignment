...
public class Regex {
    ...
    public static void main(String[] args) {
        ...
        // Provide the pattern
        pattern = Pattern.compile("(?=.*[A-Za-z])[^tmcrz]{8,}(?=.*\\d).");
        ...
        pattern = Pattern.compile("\\w{15,}(?=.*\\d).{1,}@ymail.com");
        ...
        // Provide regex pattern
        pattern = Pattern.compile("((http|www.)://)?[a-z./-]{2,}(com|ph)");
        ...
        // Provide regex pattern
        pattern = Pattern
                .compile("\\w+(.pptx)|\\w+(.docx)|\\w+(.xlsx)|\\w+(.txt)");
